document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!Array.isArray(productos) || !Array.isArray(emprendedores)) {
  ...
  const productosFiltrados = productos.filter(
    (p) => p.categoria.toLowerCase().trim() === categoriaNormalizada
  );

  if (productosFiltrados.length === 0) {
    contenedor.innerHTML =
      "<p class='text-center text-gray-600'>No hay productos en esta categoría.</p>";
    return;
  }
